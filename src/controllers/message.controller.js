...
export const getPosts = asyncWrapper(async (req, res) => {
  ...
  const [items, total] = await Promise.all([
    Message.find({ user: account._id }).sort({ createdAt: -1 }).skip(skip).limit(limit),
    Message.countDocuments({ user: account._id })
  ]);
...
export const removePost = asyncWrapper(async (req, res) => {
  ...
  const post = await Message.findOne({ _id: messageId, user: account._id });
